...
export default function AudioUpload({ showUpload, setUploadFlag }) {
  ...
  const getAudio = async () => {
    try {
      const result = await db.audio
        .where("key")
        .equals(showUpload.number)
        .toArray();
      setAudioFile(result);
    } catch (e) {
      return [];
  ...
  const convertFileToBas64 = (file) => {
    ...
    reader.onload = () => {
      ...
      db.audio.add({
        name: `${showUpload.number}audioBase64`,
        stream: result,
        key: showUpload.number,
        filename: fileName,
      });
      updateShowUpload();
    };
  ...
  return (
    <div
      className={`${style.mainContainer} ${
        showUpload.show ? style.show : style.hide
    ...
    >
      ...
      <div className={`${style.buttonscontainer} col-10 col-lg-4`}>
        <div className={`${style.closeAudioUpload} col-12 px-3 py-2`}>
          <span
            className="col-6"
            style={{ textAlign: "end", fontSize: "20px", alignItems: "center" }}
          >
            {showUpload.number}
          </span>
          <div
        ...
        <div className="col-12 my-4 d-flex justify-content-around align-items-end">
          <button className="col-4" onClick={uploadAudio}>
            {audioFile[0] ? "Replace" : "Upload"}
          </button>
          <button className="col-4" onClick={removeAudio}>
            Delete
          </button>
        </div>
      </div>
