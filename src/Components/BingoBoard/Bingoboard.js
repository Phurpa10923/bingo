...
export default function Bingoboard({drawnnumbers,setUploadFlag,showUpload,gameStarted}){
    const bingoItem = [];
    const handleAudioUpload=(event)=>{
        if(!gameStarted){
            ...
            setTimeout(()=>{
                ...
                element.style.border = element.fileAttached ?  '2px solid var(--primary-color)': '0.5px solid var(--fourth)';
            },1000)
        }
    }
    ...
    for(var i=1;i<91;i++){
        bingoItem.push(<div key={i} onClick={handleAudioUpload} className={`bingoItem number${i} ${drawnnumbers.includes(i)?"numberDrawned" :""} ${drawnnumbers[drawnnumbers.length-1]===i?"isPicked":''}`}>
            {i}
        </div>);
    }
    return(
