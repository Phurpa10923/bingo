...
export default function Navbar({handleStart , showSettingPopup}){
    ...
    return (
        <div className="navbarContainer">
            <div className="d-flex align-items-center" style={{cursor:'pointer'}}><img src={logo} alt="Logo" style={{width:'200px'}}></img></div>
            <div className="d-flex justify-content-around" style={{fontSize:'20px',cursor:'pointer'}}>
                <FaPlay className={`mx-3 ${clicked?'disabled':''}`} data-tooltip-id='play-text' onClick={clicked?()=>{}:handleClickFlagUpdate}></FaPlay>
                <IoSettings className={`${clicked?'disabled':''} mx-3`} onClick={clicked?()=>{}:showSettingPopup} data-tooltip-id='setting-text'></IoSettings>
                <FaInfo className="mx-3" data-tooltip-id="game-info"></FaInfo>
                <FaTicketAlt className="mx-3" data-tooltip-id='generate-ticket'></FaTicketAlt>
                ...
                <Tooltip id="generate-ticket" style={{textAlign:'center'}}>
                    <span >Generate tickets <br></br><span style={{color:'rgb(226,183,20)'}}>Coming soon !</span></span>
                </Tooltip>
                <Tooltip id="game-info" style={{ fontFamily: 'Robo Regular',width:'300px'}}>
                    <div>
                        ...
                        <div className= {`${gameSetting.isManual ? 'd-none':'d-flex'} row mt-2`}>
