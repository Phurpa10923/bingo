...
export default function Gamesetting({settingPopup,setPopup,setUploadFlag}){
    ...
    return(
        <div ref = {gameContainerRef} className={Style.gameSettingContainer} style={settingPopup?{transform:'translateX(0%)'}:{transform:'translateX(100%)'}}>
            <div className={Style.backpage + ' col-12 m-0 p-0 h-100'}  onClick={hideSetting}>
            </div>
            <div className={'col-4 col-sm-6 col-md-4 ' + Style.popupcontainer} >
                <div className={Style.header + ' d-flex align-item-center justify-content-between'}>
                    <div>
                        <span>Set up</span>
                ...
                <div className="row">
                    <div className='col-12 m-3 d-flex justify-content-start'>
                        ...
                        <input typeof='text' className='col-4' ref={nameRef} placeholder={gamesetting.gamename}></input>
                    </div>
                    <div className='col-12 m-3 d-flex justify-content-start'>
                        <span className='col-3'>Manual </span>
                        <label className={Style.switch}>
                            <input type="checkbox" checked={gamesetting.isManual} onClick={updateIsManualFlag}></input>
                            <span className={`${Style.slider} ${Style.round}`}></span>
                        </label>
                    </div>
                    <div className={`col-12 m-3 ${isManual?'d-none':'d-flex'} justify-content-start`}>
                        <span className='col-3'>Time </span>
                        <input type='number' ref={timerRef} className='col-4' placeholder={gamesetting.timer} onChange={onTimeEnter}></input>
                    </div>
                    <div className='col-12 m-3 d-flex justify-content-start'>
                        <span className={`${Style.saveSetting} py-2`} onClick={updateGameSettings} >Save <IoSave></IoSave></span>
                    </div>

                    <div className='col-12 m-3 d-flex justify-content-start'>
                        <span className={`${Style.resetAudio} py-2`} onClick={removeAudios} >Reset Audios <GrPowerReset></GrPowerReset></span>
                    </div>
                </div>
            </div>
        </div>
